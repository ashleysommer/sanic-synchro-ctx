...
ifeq ("$(FilePath)", "")
	poetry run black --config=./pyproject.toml --verbose sanic_synchro_ctx
	poetry run isort sanic_synchro_ctx
else
...
ifeq ("$(FilePath)", "")
	...
	poetry run black --config=./pyproject.toml --check --verbose sanic_synchro_ctx
